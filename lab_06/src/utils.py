...
def readKoeffs():
    try:
        ...
        elite_ant_total = int(input("Введите количество элитных муравьев: "))
    except:
    ...
    return alpha, beta, k_evaporation, days, elite_ant_total

def parseAntAlg():
    matrix = readMatrix()
    size = len(matrix)
    alpha, beta, k_evaporation, days, elite_ant_total = readKoeffs()
    result = antAlgorithm(matrix, size, alpha, beta, k_evaporation, days, elite_ant_total)
    print("\n\nМинимальная сумма пути = ", result[0],
            "\nПуть: ", result[1])
