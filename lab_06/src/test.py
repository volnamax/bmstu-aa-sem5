...
def testTime():
    ...
    for size in sizes:
        ...
        start = process_time()
        antAlgorithm(matrix, size, 0.5, 0.5, 0.5, 250, 10)
        end = process_time()
...
def parametrization(type = CSV):
    ...
    for alpha in alpha_arr:
        ...
        for k_eva in k_eva_arr:
            ...
            for elite_ant_total in elite_ant_total_arr:
                res1 = antAlgorithm(matrix1, size, alpha, beta, k_eva, days, elite_ant_total)
                res2 = antAlgorithm(matrix2, size, alpha, beta, k_eva, days, elite_ant_total)
                if (type == LATEX):
                    sep = " & "
                    ender = " \\\\"
                elif (type == CSV):
                ...
                str1 = "%4.1f%s%4.1f%s%4d%s%5d%s\n" \
                    % (alpha, sep, k_eva, sep, elite_ant_total, sep, res1[0] - optimal1[0], ender)

                str2 = "%4.1f%s%4.1f%s%4d%s%5d%s\n" \
                    % (alpha, sep, k_eva, sep, elite_ant_total, sep, res2[0] - optimal2[0], ender)
