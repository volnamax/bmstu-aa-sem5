...
void divideCluster(const std::vector<std::vector<double>> &points,
                   int depth,
                   int maxDepth,
                   std::vector<Cluster> &clusters) {
    if (depth == maxDepth) {
    ...
    for (const auto &point: points) {
        if (cosineDistance(point, centroids[0]) < cosineDistance(point, centroids[1])) {
            cluster1Points.push_back(point);
        }
        else {
            cluster2Points.push_back(point);
        }
    }

    // Рекурсивно разделяем каждый подкластер
    divideCluster(cluster1Points, depth + 1, maxDepth, clusters);
    divideCluster(cluster2Points, depth + 1, maxDepth, clusters);
}
